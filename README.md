...
| 步骤 | 内容 | 状态 |
|---|---|---|
| 一、形 | 汉字字形：分词器词表（当前阶段） | 🔨 进行中，第一版收尾 |
| 二、音 | 汉字拼音、发声 | ⏳ 第一版开源后启动 |
| 三、意 | 每个字/词/偏旁的含义 | ⏳ 规划中 |
...
| 阶段 | 内容 | 状态 |
|---|---|---|
| 1a | 中文原生分词器（设计 → 训练 → 评测） | ✅ 64k 词表：token/字 0.938 反超 Qwen/GPT-4o，bits/字全场第一，整字保持率 1.0 |
| 1b | 拉丁挂件模块（可切断、默认关闭） | ✅ 已实现：CompositeTokenizer + 切断测试；挂件词表质量优化列 v1.1 |
| 1c | 开源发布 | ⏳ 待用户确认名称/许可证/账号 |
| 2 | 中文优先语料管线 + 小规模预训练实验 | ⏳ 规划中 |
| 3 | 中文原生推理数据体系 | ⏳ 规划中 |
